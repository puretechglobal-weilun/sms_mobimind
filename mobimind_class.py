...
def get_keyword(country,gateway,operator,shortcode,keyword):
    ...
    #product = "egypt_" + gateway + "_" + operator + "_" + shortcode + "_" + keyword
...
def get_subscriber(country,gateway,operator,shortcode,keyword,msisdn):
    dynamodb = boto3.resource("dynamodb")
    table = dynamodb.Table("mobimind_subscriber")
    rid = country + "_" + gateway + "_" + operator + "_" + shortcode + "_" + keyword + "_" + msisdn 
    #rid = "egypt_" + gateway + "_" + operator + "_" + shortcode + "_" + keyword + "_" + msisdn 
    response = table.query(
...
def update_dn(dn_json):
    ...
    #update / create MT
    dynamodb = boto3.resource("dynamodb")
    # table = dynamodb.Table("tuma_mt")
    # response = table.update_item(
    #     Key={"mt_id": msg_id,"keyword": keyword },
    #     UpdateExpression="SET dn_status = :dn_status",
    ...
    dynamoDB_status = ""
    mt_json={}
    mt_json["mt_id"] = str(generateRandomString(8)) + "-" + str(generateRandomString(4)) + "-" + str(generateRandomString(4)) + "-" + str(generateRandomString(4)) + "-" + str(generateRandomString(12))
...
def generateRandomString(length):
    ...
    for x in range(length):
    	ran_string += char[random.randint(0,char_length) - 1]
